...
并发——同时执行多个任务——被大量用于提高吞吐量（以单个计算任务使用多处理器的方式）
或提升响应能力（方法是在程序的一部分处理业务的同时，另一部分提供响应）。
所有的现代编程语言都对并发提供了支持。
C++里有个久经20多年考验，获得现代硬件普遍支持的方式，
C++标准库对并发的支持是上述方法的一个变种，具备可移植性且类型安全。
...
至于这个内存模型，只要程序员能避免数据竞争（对可变数据的失控并发访问），
它就确保一切能够无脑地执行得丝般顺滑。
...
因为线程共享一个地址空间，它们可以通过共享对象（§15.5）进行通信。
...
此代码运行良好，而且该技术很常见，但我觉得通过引用传回结果不够优雅，
所以，§15.7.1会再聊到这个话题。
...
资深程序员会觉得这以偏概全（比如，多个任务同时读取不可变的数据就没问题），
但请考虑这个问题：对于一组给定的对象，在同一时刻，
如何确保至多只有一个任务进行访问。

解决方案的基本要素是`mutex`（互斥量），一个“互斥对象（mutual exclusion object）”。
...
无需天资聪颖就能猜到，对于一个名为`rec`的`Record`，
...
此处用了`duration_cast`将时间单位调整为所需的纳秒。

对于利用外部事件进行通信的基本支持由
`<condition_variable>`中的`condition_variable`提供。
`condition_variable`是个允许一个`thread`等待其它线程的机制。
确切地说，它允许`thread`等待某种
*条件（condition）*（通常称为*事件（event）*）的发生，
作为其它`thread`完成作业的结果。
...
标准库提供了几个构件，以便程序员能够工作在（可能需要并发的工作）任务概念的层级，
而不必直面线程和锁这么低的层级：

- `future`和`promise`从单独线程上生成的任务里重返回一个值
...
它占有其`promise`并且（间接地）要对该任务占有的资源负责。
...
这有个明显的限制：需要对共享资源加锁的任务想使用`async()`，门儿都没有。
