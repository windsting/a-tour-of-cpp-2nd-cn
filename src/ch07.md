...
它可以针对任何支持`begin()`和`end()`的数据结构调用，以便 范围`for`循环 起作用。
这类数据结构包括标准库的`vector`、`list`和`map`。
另外，该数据结构的元素类型在使用方面有限制：该类型必须能够与参数`Value`相加。
可行的例子有`int`、`double`和`Matrix`（对任何靠谱的`Matrix`而言）。
我们说`sum()`在两个维度上是通用的：用于存储元素的数据结构类型（“是个序列(sequence)”），
以及元素的类型。

因此`sum()`要求其第一个模板参数是某种序列，其第二个模板参数是某种数字。
我们把这种要求称为*概束（concept）*。

对概束的支持尚未进入ISO C++，但已经是一条ISO技术细则（Technical Specification）[ConceptsTS]。
编译器实现已经有所应用，因此，尽管其细节很可能会变动，尽管在进入生产环境代码前，
还要等待数年，我依然要冒险在此推荐它。

### 7.2.1 概束应用 {#Use-of-Concepts}

多数模板参数必须符合特定需求，以便模板能通过编译并生成正确的代码。
就是说，多数模板都是受限模板(§6.2.1)。
类型名称引入符号`typename`是最松散的约束，仅要求该参数是个类型。
通常能比这做得更好，重新考虑`sum()`例子：
...
编辑器仅根据`sum()`的接口就能驳回错误的调用，而无需查阅其实现。
这改善了错误报告。

<a class="en-page-number" id="95"></a>

但是，这份`sum()`接口的细节并不完善：我“忘了”说，需要把`Sequence`的元素于`Number`相加。
可以这样做：
...
到那时就会庆幸用了更通用的规则（此处是`Arithmetic`），而没有把需求缩窄到“可以`+=`“。

像第一个使用概束的`sum()`那样，指明部分细节已经很有用了。
只要细节不够完整，某些错误只能等到实例化期才能发现。
不过指明部分细节大有助益，表明意图，对平稳的增量开发至关重要，
也就是我们未能一开始就认清所有需求的情形。
有了完善的概束库的帮助，初步的规划就将近乎完善。

不难猜到，`requires Arithmetic<Value_type<Seq>,Num>`被称为`requirements`子句。
...
在尚且无法应用概束的场合，设计模板的时候，确保其模板参数的约束在语意上有意义是很重要的(§7.2.4)。

### 7.2.2 基于概束的重载 {#Concept-based-Overloading}

在妥善声明模板接口后，可以基于其属性进行重载，与函数重载极其类似。
...
在编译器找不到最佳匹配时，会报一个二义性错误。
基于概束的重载，其规则远比普通重载(§1.3)简单。首先考虑单个模板参数对应多个可选函数的情况：
...
不，那个`requires requires`不是误拼。前面的`requires`发起了一个`requirements`从句，
...
### 7.2.4 概束定义 {#Definition-of-Concepts}

到头来，我们想找到有用的概束，比方说库尤其是标准库里的`Sequence``Arithmetic`。
范围技术细则（Ranges Technical Specification）[RangesTS]提供了一套，
用于约束标准库算法(§12.7)。不过，简单的概束不难定义。

概束是一个编译期的谓词，用于规定一个或多个类型该如何使用。考虑这头一个最简单的例子：
...
`Equality_comparable`用来确保能够对某个类型的值进行相等或不等比较的。
...
concept Sequence = requires(S a) {
    typename Value_type<S>;             // S必须是值类型
    typename Iterator_type<S>;          // S必须是迭代器类型
...
### 7.3.1 概束的使用 {#Use-of-Concepts}

优秀、好用的概念（concept）很基础，它们被发现出来的情形多过被设计出来。
比如整数和浮点数（连古典的C语言中都有定义）、序列，以及更通用的数学概念，例如域和向量空间。
它们代表某个应用领域里的基本概念。这就是它们被称作“概念（concept）”的原因。
把概束识别并正式化，达到高效泛型编程必要的程度，极具挑战。
...
比如说：别把`+`定义成除法；对于任何合理的数字系统，它都不符合要求。
...
### 7.3.2 利用模板进行抽象 {#Abstraction-Using-Templates}

良好的抽象是从具体例子中精心培育而来的。
试图为所有构想的需求和技术做准备，并对其进行“抽象”是个馊主意；必将导致粗鄙和代码膨胀。
相反，应该以一个——最好是多个——实际使用的具体例子为开端，并努力去消除非实质的细节。考虑：
...
把具体类型换成模板参数，就可以回答前四个问题，得到标准库算法`accumulate`最简单的形式：
...
- 待遍历的数据结构被抽象成一对迭代器表示的序列(§12.1)。
- 待累加的类型被换成了参数。
- 初始值从输入获取；进行累加的类型就是初始值的类型。

一个快捷的检验或者——甚佳的——评测表明，针对多种数据结构调用生成的代码与手动编码的原版一致。
例如：
...
从一段具体代码（多段就更好了）保留性能泛化的过程叫做*提升（lifting）*。
反过来说，开发模板最好的办法通常是：
...
## 7.4 可变参数模板 {#Variadic-Templates}

模板可定义成接受任意数量任意类型的参数。
这样的模板叫*可变参数模板（variadic template）*。
考虑一个简单的函数，它任意具有`<<`运算符的类型的值。
...
此处`tail`是一个（函数参数的）参数包，其元素类型可以在（模板参数的）参数包`Tail`中找到。
因此`print()`可以接收任意类型、任意数量的参数。

调用`print()`，会把参数分成头部（首个）和尾部（其余的）。
头部会被输出，然后会为尾部再调用`print()`。
...
可变参数模板（有时候称为*变参（variadic）*）的强大之处在于，能够接收给出的任何参数。
其弱点包括：
...
### 7.4.1 折叠表达式 {#Fold-Expressions}

为简化简单可变参数模板的实现，C++17提供了一个针对参数包元素进行有限迭代的形式。例如：
...
此处，`sum()`可以接收任意类型的任意多个参数。假设`sum()`确实会将其参数相加，则有：
...
此处，`(v+...+0)`意思是把`v`的所有元素相加，从初始值`0`开始。
...
这种情况下，直接把参数复制到一个vector或者想要的容器类型，通常可以简化后续使用：
...
template<typename... Ts> int fct(Ts&&... ts)
{
...
### 7.4.2 前向参数 {#Forwarding-Arguments}

通过接口把参数原封不动传递，是可变参数模板的重要用途。
考虑一个网络输入信道，其中运送具体值的方法是个参数。
不同的传送机制拥有各自的一套构造参数：
...
## 7.5 模板编译模型 {#Template-Compilation-Model}
...
- [6] 尽可能使用标准概束（即：范围概束，Ranges concepts）；§7.2.4; [CG: T.11]。
- [7] 如果需要个简单的一次性函数对象，用lambda表达式；§6.3.2。
- [8] 模板没有分离编译：在每个用到它的编译单元，`#include`模板定义
- [9] 用模板表示容器和范围；§7.3.2; [CG: T.3]。
...
- [13] 对于类型系统的参数列表，别用可变参数模板（这种情况尽量用初值列表）；§7.4。
- [14] 使用模板时，确保其定义（而不仅仅是声明）在作用域内；§7.5。
- [15] 模板替工编译期“鸭子类型”；§7.5。

[^1] 出自一份意大利杂志《INFOMEDIA》对 Alexander A. Stepanov 的访谈录，
访谈内容全文位于 <http://www.stlport.org/resources/StepanovUSA.html>。
—— 译者注
